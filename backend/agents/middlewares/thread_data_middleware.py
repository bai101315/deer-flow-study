# ThreadDataMiddleware（线程数据中间件）, 为每个线程（thread_id）的执行创建标准化的专属目录结构
# 并通过「懒加载 / 即时加载」策略管理目录初始化时机，最终将目录路径注入 Agent 状态，供后续流程（如文件上传、数据输出、工作区操作）复用
...
class ThreadDataMiddleware(AgentMiddleware[ThreadDataMiddlewareState]):
    ...
    def _get_thread_paths(self, thread_id: str) -> dict[str, str]:
        ...
        """
        # 根据 thread_id 计算该线程的三个核心目录路径（仅计算路径，不创建目录
        # workspace_path：线程专属工作区目录（如 {base_dir}/threads/{thread_id}/user-data/workspace）
        # uploads_path：用户上传文件目录（如 {base_dir}/threads/{thread_id}/user-data/uploads）
        # outputs_path：输出文件目录（如 {base_dir}/threads/{thread_id}/user-data/outputs
        return {
