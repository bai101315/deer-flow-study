...
# 专门为 ** 异步编程（asyncio）** 设计的「上下文变量]
# 特性	普通全局变量	contextvars.ContextVar
# 并发隔离性	❌ 所有协程 / 任务共享同一个值，A 任务改了会影响 B 任务	✅ 每个异步任务有自己独立的副本，互不干扰
# 适用场景	同步代码、单任务场景	异步并发场景（如 Web 服务、多 Agent 并发请求）
import contextvars
...
class DeferredToolRegistry:
    ...
    def promote(self, names: set[str]) -> None:
        ...
        """
        # 从延迟注册表中移除工具，以便它们能够通过过滤器。
        # 1, 创建延迟注册表,存入MCP工具 
        # 2. DeferredToolFilterMiddleware 开始工作：
        #  把 registry._entries 里的所有工具，从 bind_tools 中过滤掉
        #  → LLM 此时只能看到 builtin_tools（比如 tool_search），看不到 MCP 工具
        if not names:
            return
...
def tool_search(query: str) -> str:
    ...
    # 转成OpenAI函数格式的工具列表,使得LLM能够理解并调用
